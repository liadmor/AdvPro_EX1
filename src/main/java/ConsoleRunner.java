...
public class ConsoleRunner {
    ...
    public boolean validInput (String str1){
        int wordlen = str1.length();
        if (wordlen < 1) {
            return false;
        }
        wordlen--;
        while (wordlen >= 0){
            if ((str1.charAt(wordlen) < '0')||(str1.charAt(wordlen) > '9')){
                return false;
            }
            wordlen--;
        }
    ...
    public void runGame() {
        ...
        try {
            ...
            String numIn = "";
            try{
                while (!validInput(numIn)) {
                    out.println("You must enter a number!");
                    numIn = in.readLine();
                }
            }catch (IOException ioe){ //in order to catch input field specified.
                ioe.printStackTrace();
            }
            NUM_PLAYERS = Integer.parseInt(numIn);
            err.println(e);
            return;
        }
        /*try {
